...
// =========================================================================
// PHẦN 11: SMOOTH SCROLL PAST LINES MANAGER (FALLBACK CHO GLASSYFLOW, NEW ENGINE)
// Mô tả: Đánh dấu .blyrics--gf-past cho các dòng đã hát xong sau 300ms delay (giống GlassyFlow)
// =========================================================================
(function () {
  'use strict';
  ...
  function updatePastLines(container) {
    if (container.classList.contains('blyrics--gf-managed')) return;

    const lines = Array.from(container.querySelectorAll('.blyrics--line'));
    let activeIndex = -1;
    ...
    _delayTimer = setTimeout(() => {
      ...
      for (let i = 0; i < lines.length; i++) {
        if (i < activeIndex) {
          lines[i].classList.add('blyrics--gf-past');
        } else {
          lines[i].classList.remove('blyrics--gf-past');
        }
      }
    }, 300);
  }
  ...
  const observer = new MutationObserver(() => {
    ...
    if (container) updatePastLines(container);
  });
